...
func TestAuthDeleteUser(t *testing.T) {
	...
	_, err := client.Auth.DeleteUser(context.TODO())
	if err != nil {
...
func TestAuthUserTokenWithOptionalParams(t *testing.T) {
	baseURL := "http://localhost:4010"
	if envURL, ok := os.LookupEnv("TEST_API_BASE_URL"); ok {
		baseURL = envURL
	}
	...
	_, err := client.Auth.UserToken(context.TODO(), hyperspell.AuthUserTokenParams{
		UserID:    "user_id",
		ExpiresIn: hyperspell.String("30m"),
		Origin:    hyperspell.String("origin"),
	})
	if err != nil {
		var apierr *hyperspell.Error
		if errors.As(err, &apierr) {
