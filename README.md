...
```go
import (
	"github.com/stainless-sdks/hyperspell-go" // imported as hyperspell
)
```
...
The hyperspell library uses the [`omitzero`](https://tip.golang.org/doc/go1.24#encodingjsonpkgencodingjson)
...
See the [full list of request options](https://pkg.go.dev/github.com/stainless-sdks/hyperspell-go/option).
...
`*hyperspell.Error`. This contains the `StatusCode`, `*http.Request`, and
...
We also provide a helper `hyperspell.File(reader io.Reader, filename string, contentType string)`
